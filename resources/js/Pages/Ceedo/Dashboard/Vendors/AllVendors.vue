...
function filtersearchInput() {
    router.get(route('ceedo.vendors'),
          ...
          <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              <tr v-for="vendor in vendors" :key="vendor.id" class="text-gray-700 dark:text-gray-400 hover:bg-gray-100 cursor-pointer" @click="goToRoute(vendor.profile.id)">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center text-sm">
                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                      <img v-if="vendor.profile.image" class="object-cover w-8 h-8 rounded-full" :src="'/images/clients/' + vendor.profile.image" alt=""
                      aria-hidden="true" />
                    ...
                    <div>
                      <p class="font-semibold">{{ getFullName(vendor.profile.first_name, vendor.profile.middle_name_name, vendor.profile.last_name) }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-4 py-3 text-sm">
                ...
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ vendor.profile.user.mobile_number }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
              ...
              <tr v-if="vendors.length <= 0" class="text-center">
                <td colspan="9" class="p-2">No Vendors Yet!</td>
              </tr>
            </tbody>
          </table>
        </div>
