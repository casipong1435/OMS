...
function openApproveResponseModal() {
    response.value = 1;
    responseModal.value = true;
}

function openDeclineResponseModal() {
    response.value = 2;
    responseModal.value = true;
}
...
function submitResponse() {
    if (response.value == 1) {
        router.put(route('ceedo.approveApplication', usePage().props.profile.business[0].id), {}, {
            onSuccess: (page) => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
                ...
                window.location.href = route('ceedo.vendors');
            }
        });
    } else {
        router.put(route('ceedo.declineApplication', usePage().props.profile.business[0].id), {remarks: remarks.value}, {
            onSuccess: page => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
    ...
    <Layout>
        <div class="container px-6 mx-auto grid">
            ...
            <div class="container mx-auto p-6 bg-white rounded-lg shadow-lg">
                <div v-if="profile.business[0].status == 0" class="flex justify-end items-center gap-3 mb-7">
                    <button type="button" class="btn btn-success rounded-none"
                        @click="openApproveResponseModal()">Approve</button>
                    <button type="button" class="btn btn-error rounded-none"
                        @click="openDeclineResponseModal">Decline</button>
                </div>
                <div v-else class="flex justify-end items-center gap-3 mb-7 cursor-pointer hover:opacity-90">
                    <label for="statusModal" class="badge badge-error">{{ profile.business[0].status == 2 ? 'Rejected' : 'Declined' }}</label>
                </div>
                <hr>
                    ...
                    <div class="grid grid-cols-3 gap-5">
                        ...
                        <div class="col-span-3 sm:col-span-2 md:col-span-1">
                            <div class="flex flex-col">
                                ...
                                <input type="text" :value="profile.business[0].establishment_unit.establishment.area.name"
                                    class="w-full mt-2 p-3 border rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                    id="payment_cycle" disabled>
                            </div>
                        </div>
            ...
            <div class="modal">
                ...
                <div class="modal-content flex flex-col gap-5">
                    ...
                    <div class="flex justify-center items-center flex-col gap-2" :class="{'text-success':response == 1, 'text-error':response == 2}">
                        <span>
                        ...
                        <span class="text-gray-700">Are you sure you want to {{ response == 1 ? 'approve' : 'decline' }} this application?</span>
                    </div>
                    <div class="my-3" v-if="response == 2">
                        <label for="remarks">Reason:</label>
                        <textarea id="remarks" v-model="remarks" class="w-full" required></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button class="btn btn-block" :class="{'btn-success':response == 1, 'btn-error':response == 2}" @click="submitResponse()">Confirm</button>
            ...
            <div class="modal !h-full" style="z-index: 99999; margin-top: 0;">
                ...
                <div class="modal-content">
                    ...
                    <div v-if="profile.business[0].status == 2 || profile.business[0].status == 3">
                        ...
                        v-if="profile.business[0].status == 2 || profile.business[0].status == 3" :value="profile.business[0].remarks">
                        </textarea>
                    </div>
