...
function submitResponse() {
    if (response.value == 1) {
        router.put(route('ceedo.approveApplication', usePage().props.profile.business.id), {}, {
            onSuccess: (page) => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
        ...
        router.put(route('ceedo.declineApplication', usePage().props.profile.business.id), {remarks: remarks.value}, {
            onSuccess: page => {
                Swal.fire({
                    toast: true,
                    icon: 'success',
    ...
    <Layout>
        <div class="container px-6 mx-auto grid">
            ...
            <div class="container mx-auto p-6 bg-white rounded-lg shadow-lg">
                <div v-if="profile.business.status == 0" class="flex justify-end items-center gap-3 mb-7">
                ...
                <div v-else class="flex justify-end items-center gap-3 mb-7 cursor-pointer hover:opacity-90">
                    <label for="statusModal" class="badge badge-error">{{ profile.business.status == 2 ? 'Rejected' : 'Declined' }}</label>
                </div>
                <hr>
