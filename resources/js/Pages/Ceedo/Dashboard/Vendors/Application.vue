...
function filtersearchInput() {
    router.get(route('ceedo.applications'),
...
function goToRoute(id){
  window.location.href = route('ceedo.applicationInfo', id);
}
        ...
        <div class="w-full overflow-x-auto">
          ...
          <table class="w-full whitespace-no-wrap">
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
              <tr v-for="applicant in applicants" :key="applicant.id" class="text-gray-700 dark:text-gray-400 hover:bg-gray-100 cursor-pointer" @click="goToRoute(applicant.profile.id)">
                ...
                <td class="px-4 py-3">
                  <div class="flex items-center text-sm">
                    <div class="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                      <img v-if="applicant.profile.image" class="object-cover w-8 h-8 rounded-full" :src="'/images/clients/' + applicant.profile.image" alt=""
                      aria-hidden="true" />
                    ...
                    <div>
                      <p class="font-semibold">{{ getFullName(applicant.profile.first_name, applicant.profile.middle_name_name, applicant.profile.last_name) }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.profile.barangay.brgyDesc }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.profile.user.mobile_number }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.kind_of_business }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.establishment_unit.establishment.area.name }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.establishment_unit.establishment.name }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
                    {{ applicant.establishment_unit.id }}
                  </span>
                </td>
                <td class="px-4 py-3 text-sm">
                  <span class="px-2 py-1 font-semibold rounded-full dark:bg-green-700 dark:text-green-100">
              ...
              <tr v-if="applicants.length <= 0" class="text-center">
                <td colspan="9" class="p-2">No Applicants Yet!</td>
              </tr>
            </tbody>
          </table>
        </div>
