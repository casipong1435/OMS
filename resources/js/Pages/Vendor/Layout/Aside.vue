...
const markAsRead = (id) => {
  router.post(route('user.markasReadNotification', id));
};

const markAsReadAll = () => {
  router.post(route('user.markasReadAllNotification'));
};
...
<template>
  <aside class="z-20 hidden w-64 overflow-y-auto bg-white dark:bg-gray-800 md:block flex-shrink-0">
    <div class="py-4 text-gray-500 dark:text-gray-400">
      <Link :href="route('user.dashboard')" aria-label="logo" class="ml-6 flex items-center space-x-2">
        ...
        <ul class="flex items-center flex-shrink-0 space-x-6">
          ...
          <li class="relative">
            <button class="align-middle rounded-full focus:shadow-outline-purple focus:outline-none"
              ...
              <img v-if="usePage().props.auth.image" class="object-cover w-8 h-8 rounded-full"
              ...
              <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                ...
                <li class="flex">
                  <Link
                    ...
                    :href="route('welcome')">
                  <svg class="w-4 h-4 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                ...
                <li v-if="$page.props.auth.user.status == 0" class="flex">
