...
class SMSNotificationServices
{
    ...
    public function SendPaymentReminder($response)
    {
        $message = $this->twilio->messages->create(
            ...
            [
                'body' =>
                    'Payment Reminder from CEEDO! Please pay your payment for ' . $response['due_date'] . ' amounting to ₱' . $response['amount'] . ', and Penalty ₱' . $response['penalty'] . ' to avoid penalty or business closure. Check your account for more info!',
                'from' => $this->phone,
            ]
        );
    }
    ...
    {
        $sum = (float) str_replace(',', '', $response['amount']) + $response['penalty'];
        $message = $this->twilio->messages->create(
            '+63' . $response['recepient'],  // To
            [
                'body' =>
                    'Payment Successful! You paid your payment for ' . $response['due_date'] . ' amounting to ₱' . $sum . ', paid at ' . $response['paid_at'] . '.',
                'from' => $this->phone,
            ]
        );
    }
    ...
    public function SendPermitUpdateResponse($recepient, $status, $reason)
    {
        if ($status == 1) {
            $message = $this->twilio->messages->create(
                '+63' . $recepient,  // To
                [
                    'body' =>
                        'Congratulations! Your Permit Update Request has been approved.',
                    'from' => $this->phone,
                ]
            );
        } else {
            $message = $this->twilio->messages->create(
                '+63' . $recepient,  // To
                [
                    'body' => 'Sorry! Your Permit Update Request has been declined due to ' . $reason. '.',
                    'from' => $this->phone,
                ]
            );
        }
